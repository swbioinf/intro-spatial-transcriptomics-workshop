...
```{r}
# Define a filter
so$qc<- if_else(so$nCount_RNA >= min_ncount_rna & so$percent_neg <= max_percent_neg, "Keep", "Remove")
...
Code blocks are provided for the above steps.  

```{r}
# Default thresholds that will keep all cells
min_ncount_rna <- 0
max_percent_neg <- Inf
```

...
To run this quickly, going to arbitraily reduce the number of variable features to just 10. In real life, we might pick all our VariableFeatures.
...
What to the top 3 gene look like on the tissue? How can we interpret this?
...
Lets run the test on every sample. This is slow, and could be parallelised. 
...
MoransI scores are between -1 (anticorrelated) and 1 (preferctly correlated) - 0 is random. The baseline isn't 0 - probably because the observations are already limited to the tissue. But there's still clearly some genes with higher scores than others, even though the variation between samples is quite high.
...
There's a big clump of CCL21 at the bottom of HC_a, lets figure out which FOV that is and zoom in further.
```{r message=FALSE, warning=FALSE}
...
Now we can practically bring back the cluster labels.
```{r message=FALSE, warning=FALSE}
ImageDimPlot(subset(so.HC_a, fov==4),
...
<!-- ## Co-occurance tests? (Potential addition) -->

<!-- Try to test foc co-occurance of CCL21 and cluuster (or MS4A1) -->

<!-- Easiest way might be a conversion to SFE then use voyager as per the bivariat stats workflow: https://pachterlab.github.io/voyager/articles/bivariate.html -->

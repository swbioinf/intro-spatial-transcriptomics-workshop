...
* Evaluate what type of cell might be in each cluster using multiple lines of evidence:
   + Genes that are expressed specifically in the cluster
   + Find a public dataset of colon samples, and use a 'cell typing' tool to classify cells according to its categories
   + Location of clusters within tissue
   + Discuss value of histology images 
* Make an informed choice on a celltype label for each cluster, and it for downstream analysis.
...
## Data loaing, and reproducability

Clustering (and UMAP) is a non-deterministic method - though packages can use a non-random 'seed' to make sure you get the same clusters.
But any slightly difference (e.g. removing a single cell) can result in a different result. Likewise, different underlying packages can also generate different clusters on the same data. 

These workshop materials were developed on a server with an older version of R and Seurat - and produce slightly different clusters than the workshop environment. So all our results match, we will reload from preprepared dataset.

```{r}
clustered_seurat_object_file <- here("data", "GSE234713_CosMx_IBD_seurat_60_clustered_preprepared.RDS")
so    <- readRDS(normalised_seurat_object_file) 
so    <- UpdateSeuratObject(so) # Data generated with older version of Seurat, this function updates it.
...
You can get some more information form these marker lists by dropping them into a tool like [enrichr](https://maayanlab.cloud/Enrichr/) and considering cell type enrichment.
...
This code will load, subset and convert the raw data. We will not run it here. 
```{r message=FALSE, warning=FALSE}
...
Image showing colonic mucosa, from https://www.pathologyoutlines.com/topic/colonhistology.html
![](images/colonic_mucosa.png){width=100%}

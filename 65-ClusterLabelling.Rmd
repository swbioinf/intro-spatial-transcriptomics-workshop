...
# Subset to 20% of the dataset to make it run better in workshop.
# Would use the whole dataset in a real analysis.
ncells <- ncol(sce.ts_intestine)
sce.ts_intestine.mini <- sce.ts_intestine[,sample(ncells, round(ncells*0.2))]

ts_largeintestine_sce_file <- file.path("data/tabula_sapiens/tabula_sapiens_large_intestine_mini")
saveHDF5SummarizedExperiment(sce.ts_intestine.mini, ts_largeintestine_sce_file)
```

```{r}
library(HDF5Array)
ts_largeintestine_sce_file <- file.path("data/tabula_sapiens/tabula_sapiens_large_intestine_mini")
sce.ts_intestine <- loadHDF5SummarizedExperiment(ts_largeintestine_sce_file)
```

...
* Define groups of cells with similar transcriptional profiles, that represent the celltypes in our experiment


## Overview


![](images/workflows/05_clustering.png){width=100%}
...
* Try another clustering resolution
* Plot it on the UMAP, and/or another clustree.

Share your results in the slack.
...
What does it look like on the tissue? Our clusters are based purely on transcriptional profiles.
...
And check that our clusters are balanced across samples. What is happening with CD_b?
...
```{r eval=F}
saveRDS(so, file = here("data", "GSE234713_CosMx_IBD_seurat_60_clustered_inworkshop.RDS"))
```
...
Here we are using method based purely on each cells transcriptome. You may perfer to investigate the use of a spatially-aware clustering tool such as [semanticST](https://arxiv.org/abs/2506.11491) or others (a rapidly progressing feild).
